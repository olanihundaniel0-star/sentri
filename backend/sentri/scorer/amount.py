...
def recent_prior_amount_windows(
    profile: UserProfile, event: TransactionEvent
) -> tuple[list[int], list[int]]:
    """Split a profile's transaction amounts into recent (0-30d) and prior (31-90d) windows, relative to the event's own time."""
    reference = ingest_timestamp(event.timestamp)

    recent: list[int] = []
    prior: list[int] = []
    for tx in all_transactions(profile):
...
def _amount_drift_ratio(profile: UserProfile, event: TransactionEvent) -> Optional[float]:
    """Ratio of recent (0-30d) to prior (31-90d) amount volatility."""
    recent, prior = recent_prior_amount_windows(profile, event)

    if len(recent) < MIN_WINDOW_SAMPLES or len(prior) < MIN_WINDOW_SAMPLES:
        return None
